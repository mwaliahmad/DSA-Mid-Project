...
class ScraperThread(QThread):
    ...
    def run(self):
        ...
        urls_list = self.loadLinksFromFile()
        startingIndex = self.loadProgess("Scraping/progress.txt")
        urls_list = urls_list[startingIndex:10]
        ...
        if not os.path.exists("VideosINFO.csv"):
            pd.DataFrame(columns=fieldnames).to_csv("VideosINFO.csv", index=False)

        # scrap data from each video in list
        for index, url in enumerate(urls_list, startingIndex + 1):
            if self.is_stopped:
                break

            while self.is_paused:
                self.msleep(1000)  # Pause the thread for 1 second
                if self.is_stopped:
                    break

            # Calculate the progress percentage
            progress_percentage = int((index / total_urls) * 100)
            self.data_ready.emit(progress_percentage)
            self.ScrapVideo(url, index)

        if not self.is_stopped:
            self.data_ready.emit(100)  # Indicate that scraping is complete
    ...
    def ScrapVideo(self, videoLink, startingIndex):
        ...
        try:
            channel_name = soup.find(
                "a",
                attrs={"class": "yt-simple-endpoint style-scope yt-formatted-string"},
            )
        ...
        df.to_csv("VideosINFO.csv", mode="a", header=False, index=False)

        f = open(file="Scraping/progress.txt", mode="w")
        f.write(str(startingIndex) + "\n")

    def loadLinksFromFile(self):
